<template>
<div class="container">

    <div class="grid grid-cols-3 gap-x-5">

        <div class="container">
            <div class="container">
                <img class="rounded-md" src="../assets/mokup_img.jpg" alt="">
            </div>
            <div class="flex justify-between my-3">
                <h1 class= "font-bold" >Bodegón Nuñez</h1>
                <img src="../assets/person_outline_black_24dp.svg" alt="">
            </div>
            <div class="container my-2">
                <img src="" alt="">
                <p class="text-left text-sm">Palermo, CABA</p>
            </div>
            <div class="container">
                <p class="text-left">Un equilibrio unico entre el estilo moderno y platos tipicos porteños. Te invitan a disfrutar de esta experiencia en la comida tu living</p>
            </div>
            <div class="container my-2">
                <p class="text-lg font-mediun text-left">5500 puntos</p>
            </div>
        </div>
    
        <div class="container">
            <div class="container">
                <img class="rounded-md" src="../assets/mokup_img.jpg" alt="">
            </div>
            <div class="flex justify-between my-3">
                <h1 class= "font-bold" >Bodegón Nuñez</h1>
                <img src="../assets/person_outline_black_24dp.svg" alt="">
            </div>
            <div class="container my-2">
                <img src="" alt="">
                <p class="text-left text-sm">Palermo, CABA</p>
            </div>
            <div class="container">
                <p class="text-left">Un equilibrio unico entre el estilo moderno y platos tipicos porteños. Te invitan a disfrutar de esta experiencia en la comida tu living</p>
            </div>
            <div class="container my-2">
                <p class="text-lg font-mediun text-left">5500 puntos</p>
            </div>
        </div>

        <div class="container">
            <div class="container">
                <img class="rounded-md" src="../assets/mokup_img.jpg" alt="">
            </div>
            <div class="flex justify-between my-3">
                <h1 class= "font-bold" >Bodegón Nuñez</h1>
                <img src="../assets/person_outline_black_24dp.svg" alt="">
            </div>
            <div class="container my-2">
                <img src="" alt="">
                <p class="text-left text-sm">Palermo, CABA</p>
            </div>
            <div class="container">
                <p class="text-left">Un equilibrio unico entre el estilo moderno y platos tipicos porteños. Te invitan a disfrutar de esta experiencia en la comida tu living</p>
            </div>
            <div class="container my-2">
                <p class="text-lg font-mediun text-left">5500 puntos</p>
            </div>
        </div>

        <div class="container">
            <div class="container">
                <img class="rounded-md" src="../assets/mokup_img.jpg" alt="">
            </div>
            <div class="flex justify-between my-3">
                <h1 class= "font-bold" >Bodegón Nuñez</h1>
                <img src="../assets/person_outline_black_24dp.svg" alt="">
            </div>
            <div class="container my-2">
                <img src="" alt="">
                <p class="text-left text-sm">Palermo, CABA</p>
            </div>
            <div class="container">
                <p class="text-left">Un equilibrio unico entre el estilo moderno y platos tipicos porteños. Te invitan a disfrutar de esta experiencia en la comida tu living</p>
            </div>
            <div class="container my-2">
                <p class="text-lg font-mediun text-left">5500 puntos</p>
            </div>
        </div>
    
        <div class="container">
            <div class="container">
                <img class="rounded-md" src="../assets/mokup_img.jpg" alt="">
            </div>
            <div class="flex justify-between my-3">
                <h1 class= "font-bold" >Bodegón Nuñez</h1>
                <img src="../assets/person_outline_black_24dp.svg" alt="">
            </div>
            <div class="container my-2">
                <img src="" alt="">
                <p class="text-left text-sm">Palermo, CABA</p>
            </div>
            <div class="container">
                <p class="text-left">Un equilibrio unico entre el estilo moderno y platos tipicos porteños. Te invitan a disfrutar de esta experiencia en la comida tu living</p>
            </div>
            <div class="container my-2">
                <p class="text-lg font-mediun text-left">5500 puntos</p>
            </div>
        </div>

        <div class="container">
            <div class="container">
                <img class="rounded-md" src="../assets/mokup_img.jpg" alt="">
            </div>
            <div class="flex justify-between my-3">
                <h1 class= "font-bold" >Bodegón Nuñez</h1>
                <img src="../assets/person_outline_black_24dp.svg" alt="">
            </div>
            <div class="container my-2">
                <img src="" alt="">
                <p class="text-left text-sm">Palermo, CABA</p>
            </div>
            <div class="container">
                <p class="text-left">Un equilibrio unico entre el estilo moderno y platos tipicos porteños. Te invitan a disfrutar de esta experiencia en la comida tu living</p>
            </div>
            <div class="container my-2">
                <p class="text-lg font-mediun text-left">5500 puntos</p>
            </div>
        </div>

        <div class="container">
            <div class="container">
                <img class="rounded-md" src="../assets/mokup_img.jpg" alt="">
            </div>
            <div class="flex justify-between my-3">
                <h1 class= "font-bold" >Bodegón Nuñez</h1>
                <img src="../assets/person_outline_black_24dp.svg" alt="">
            </div>
            <div class="container my-2">
                <img src="" alt="">
                <p class="text-left text-sm">Palermo, CABA</p>
            </div>
            <div class="container">
                <p class="text-left">Un equilibrio unico entre el estilo moderno y platos tipicos porteños. Te invitan a disfrutar de esta experiencia en la comida tu living</p>
            </div>
            <div class="container my-2">
                <p class="text-lg font-mediun text-left">5500 puntos</p>
            </div>
        </div>
    
        <div class="container">
            <div class="container">
                <img class="rounded-md" src="../assets/mokup_img.jpg" alt="">
            </div>
            <div class="flex justify-between my-3">
                <h1 class= "font-bold" >Bodegón Nuñez</h1>
                <img src="../assets/person_outline_black_24dp.svg" alt="">
            </div>
            <div class="container my-2">
                <img src="" alt="">
                <p class="text-left text-sm">Palermo, CABA</p>
            </div>
            <div class="container">
                <p class="text-left">Un equilibrio unico entre el estilo moderno y platos tipicos porteños. Te invitan a disfrutar de esta experiencia en la comida tu living</p>
            </div>
            <div class="container my-2">
                <p class="text-lg font-mediun text-left">5500 puntos</p>
            </div>
        </div>

        <div class="container">
            <div class="container">
                <img class="rounded-md" src="../assets/mokup_img.jpg" alt="">
            </div>
            <div class="flex justify-between my-3">
                <h1 class= "font-bold" >Bodegón Nuñez</h1>
                <img src="../assets/person_outline_black_24dp.svg" alt="">
            </div>
            <div class="container my-2">
                <img src="" alt="">
                <p class="text-left text-sm">Palermo, CABA</p>
            </div>
            <div class="container">
                <p class="text-left">Un equilibrio unico entre el estilo moderno y platos tipicos porteños. Te invitan a disfrutar de esta experiencia en la comida tu living</p>
            </div>
            <div class="container my-2">
                <p class="text-lg font-mediun text-left">5500 puntos</p>
            </div>
        </div>

    </div>
</div>
</template>